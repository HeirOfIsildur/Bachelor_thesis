...
\begin{definice}[Jednoduchá náhodná procházka v $\mathbb{Z}$]\label{def01:1}
  Nechť $\left\{ X_{i}\right\}_{i=1}^\pinf$ je posloupnost nezávislých stejně rozdělených náhodných veličin s hodnotami v $\left\{-1, +1 \right\}$, které pro každé $i \in \N$ splňují $P(X_i=1)=p \in (0,1)$ a $P(X_i=-1)=q=1-p,$. Položme $S_0\dosad0$, potom pro každé $n \in \N$ označme $S_n\dosad \suma[i][1][n]X_i$. Potom dvojici $\left(  \left\{ S_i\right\}_{i=0}^{\pinf}, p \right)$ nazýváme
    \emph{jednoduchou náhodnou procházkou v $\Z$}.
    Pokud navíc $p=q=\frac{1}{2}$, nazýváme dvojici $\left(  \left\{ S_i\right\}_{i=0}^{\pinf}, p \right)$ \emph{symetrickou náhodnou procházkou}
\end{definice}

\begin{veta}[Počet cest z počátku do bodu $(x,n)$]\label{veta01:1}
