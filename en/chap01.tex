...
\end{proof}
\begin{defn}[Return to origin]\label{defn-return_origin1}
...
\begin{proof}
 ...
 %druhá rovnost $\pr \left( X_n=1 \right) =p$ 4. rovnost \ref{thm-probability-of-maximum}}
\end{proof}
...
\begin{thm}\label{thm-return_origin_upto_time}
 Let \rw be a symmetric random walk. The probability that the last return to origin up to \Time $2n$ occurred \intime $2k$ is $\pr \left( S_{2k}=0 \right) \pr \left( S_{2 \left( n-k \right) }=0 \right) $.
\end{thm}
\begin{proof}
...
\begin{lemma}\label{thm-probability_strictly_above}
 Let \rw be a symmetric random walk. $\pr \left( S_1, S_2, \ldots, S_{2n}>0 \right) =\h \pr \left( S_{2n}=0 \right) =\h u_{2n}.$
\end{lemma}
\begin{proof}
...
\begin{proof}
 ...
 %1-\ref{thm-probability_strictly_above}
 %2-
 %3-independent X_1
 %4-\ref{thm-temporal_homogeneity}
 %5-\ref{thm-spatial_homogeneity}
 %6-S_{2n-1}\geq 0 \implies S_{2n-1}\geq 1 \implies S_{2n}\geq 0
\end{proof}
...
\begin{proof}
 ...
 %*\ref{thm-temporal_homogeneity}
 %**\ref{thm-probability_no_return}
\end{proof}
\begin{defn}[Time spend on the positive and negative sides]
 Let \rw be a random walk. We say that the walk spent $\tau$ time units of $n$ on the positive side if $\suma \indikator{S_i>0 \lor S_{i-1}>0}=\tau$. Let $\beta_{n} \left( \tau \right) $ denote the probability of such an event. We say that the walk spent $\zeta$ time units of $n$ on the negative side if $\suma\indikator{S_i<0 \lor S_{i-1}<0}=\zeta$.
\end{defn}
\begin{thm}[Arcsine law for sojourn times]\label{thm-arcsine_sojourn_times}
 Let \rw be a symmetric random walk. Then $\beta_{2n} \left( 2k \right) =\alpha_{2n} \left( 2k \right) $.
\end{thm}
\begin{proof}%Musím tohle dělat celé? Nestačilo by se odkázat na Fellera?
 Firstly let us start with degenerate cases. $\beta_{2n} \left( 2n \right) =\pr \left( \suma[i][1][2n] \indikator{S_i>0 \lor S_{i-1}>0}=2n \right) =\pr \left( S_1, S_2, \ldots, S_{2n}\geq 0 \right) =*u_{2n}$. By symmetry $\beta_{2n} \left( 0 \right) =\beta_{2n} \left( 2n \right) =u_{2n}$.
 ...
 %*-\ref{thm-probability_above_or_on}
 %*a-úplná past
 %*b rozklad neq=<+>
 %*c násobení pastí
 %*d využití odmínky na kladnost/zápornost prvních 2r kroků
 %*e \ref{thm-temporal_homogeneity}
\end{proof}
